...
export function CreateCollection({ CustomButton }: { CustomButton?: ReactNode }) {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    console.log(values)
    await postFetcher("/collection", values)
      .finally(() => {
        form.reset()
        handleOpen();
        toast({
                    ...
                    collectionPictures.map(picture =>
                      <button
                        ...
                        className={cn("rounded-full w-16 h-16 p-2 hover:bg-gray-100",
                          form.getValues().picture == picture ? 'bg-gray-200 border-4 border-gray-400 ' : 'border-2')}
                      >
