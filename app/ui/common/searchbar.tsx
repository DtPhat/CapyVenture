...
export const HomeSearchBar = ({ givenKeyword, handleEnter, placeholder = 'Search videos, stories, games...', autoFocus, fullRounded = true }: SearchBarProps) => {
  fullRounded = fullRounded || true
  const [keyword, setKeyword] = useState(givenKeyword || "")
  const searchRef = useRef<HTMLInputElement>(null)
  autoFocus && useEffect(() => {
  ...
  return (
      <div className={`flex items-center border-4 border-white focus-within:border-white relative bottom-1 flex-1 w-full cursor-pointer ${fullRounded ? 'rounded-xl' : 'rounded-t-xl'} `}>
        <label htmlFor='search' className='absolute left-2'>
          <MagnifyingGlassIcon className='w-7 h-7 cursor-pointer text-white/50 stroke-2'/>
        </label>
        <input className='bg-transparent rounded-xl focus:outline-none focus:bg-gray w-full p-2 text-white/90 dark:text-white px-12 text-xl' placeholder={placeholder} id='search' autoComplete='off'
          value={keyword} onChange={(e) => setKeyword(e.target.value)} onKeyDown={handleEnter} ref={searchRef} />
